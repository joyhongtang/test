package com.joyhong.test.control.dlroundmenuview;
...
import com.joyhong.test.control.TestRecord;
import com.joyhong.test.control.dlroundmenuview.Interface.OnMenuClickListener;
import com.joyhong.test.control.dlroundmenuview.Interface.OnMenuLongClickListener;
import com.joyhong.test.control.dlroundmenuview.utils.DLMathUtils;
import com.joyhong.test.control.dlroundmenuview.utils.DrawableUtils;
...
public class DLRoundMenuView extends View {
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        // 释放内存，回收资源
        a.recycle();
        keyCodeMap.put(KeyEvent.KEYCODE_DPAD_UP, new TestRecord(0));
        keyCodeMap.put(KeyEvent.KEYCODE_DPAD_LEFT,  new TestRecord(3));
    ...
    private ArrayList<Integer> clickedPos = new ArrayList<>();
        ...
        if (mRoundMenuNumber > 0) {
            ...
            for (int i = 0; i < mRoundMenuNumber; i++) {
                ...
                if (clickedPos.contains(i)) {
                    paint.setColor(mRoundMenuSelectedBackgroundColor);
                }
                if (!clickedPos.contains(onClickState)) {
                    clickedPos.add(onClickState);
                }
...
//                invalidate();
    ...
    public void receiverKeyDown(int keyCode,boolean needRefresh) {
        try {
            ...
            boolean testOK = true;
            Iterator<Map.Entry<Integer, TestRecord>> iter = keyCodeMap.entrySet()
                    .iterator();
            while (iter.hasNext()) {
                Map.Entry entry = (Map.Entry) iter.next();
                TestRecord record = (TestRecord) entry.getValue();
                if(!record.isSuccess()){
                    testOK = false;
                    break;
                }
            ...
            if(testOK){
                BaseTestActivity baseActivity = (BaseTestActivity) getContext();
                TestEntity testEntity = testResult.get(TestConstant.PACKAGE_NAME + baseActivity.getLocalClassName());
